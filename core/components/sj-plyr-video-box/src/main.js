...
const SjPlyrVideoBox = function (options, callback) {
  ...
  if (typeof Promise !== 'undefined') {
    return new Promise((resolve, reject) => { // eslint-disable-line
      currentBox = {
        options: _.merge({}, defaults, SjPlyrVideoBox.defaults, options),
        callback: callback,
        resolve: resolve,
        reject: reject,
      }
...
SjPlyrVideoBox.setDefaults = defaults => {
  SjPlyrVideoBox.defaults = defaults
}

SjPlyrVideoBox.open = (file, title = '视频播放', options = {}) => {
  return SjPlyrVideoBox(_.merge({
    title: title,
    file,
  }, options))
}

SjPlyrVideoBox.close = () => {
  instance.doClose()
  instance.visible = false
  currentBox = null
}

SjPlyrVideoBox.destroy = () => {
  if (instance) {
    instance.$el.remove()
    instance.$destroy()
    instance = null
