<template>
  <div v-title="$route.meta.title">
    ...
    <sj-panel>
      <span slot="header">多层表单嵌套验证实现</span>
      <el-form ref="form"
               :model="form"
               :rules="rules"
               size="medium"
        ...
        <div class="re-el-form__flex">
          <div class="medium">
            ...
            <el-form-item label="列表"
                          prop="goods">
              <demo-multiple-form ref="goods"
                                  v-model="form.goods"></demo-multiple-form>
            </el-form-item>
          </div>
        </div>
        <el-form-item>
...
<script>
import rules from '../rules/demoNestedForm.rules'
import DemoMultipleForm from '../components/DemoMultipleForm'

export default {
  components: {DemoMultipleForm},
  mixins: [rules],
  data () {
    return {
      id: 0,
