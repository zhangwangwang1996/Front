    ...
    <sj-panel>
      ...
      <el-form ref="form"
        ...
        <div class="re-el-form__flex">
          <div class="medium">
            <el-form-item label="imagBrower名称"
                          prop="name">
...
<script>
import { formRules } from '../imag-brower.rules.js'
...
export default {
  ...
  methods: {
    ...
    create () {
      request.common('createImagBrowerData', this.getForm()).then((res) => {
        this.$confirm(this.$route.meta.title + '成功, 是否继续添加？', '提示', {type: 'success'}).then(() => {
          this.$refs.form.resetFields()
        }, () => {
          this.$router.push({path: '/imag-brower/table/'})
        })
      })
    },
    update (id) {
      request.common('updateImagBrowerData', {id, ...this.getForm()}).then((res) => {
